...
exports.ChatSearchPanel = void 0;
const vscode = __importStar(require("vscode"));
const bm25_1 = require("./bm25");
const chatExtractor_1 = require("./chatExtractor");
const workspaceChatInjector_1 = require("./workspaceChatInjector");
class ChatSearchPanel {
    static createOrShow(extensionPath) {
        ...
        const panel = vscode.window.createWebviewPanel('chatSearcher', 'Chat Searcher', column, {
            enableScripts: true,
    ...
    constructor(panel, extensionPath) {
        this.disposables = [];
        this.bm25Index = new bm25_1.BM25Index();
    ...
    async ensureIndex() {
        ...
        try {
            const docs = await (0, chatExtractor_1.extractDocumentsForSearch)(this.extensionPath, (msg) => {
            ...
            this.bm25Index = new bm25_1.BM25Index(1.5, 0.75);
    ...
    async handleSearch(query, sort, workspaceFilter) {
        if (!this.isIndexed) {
            await this.ensureIndex();
        }
        if (!query.trim()) {
        ...
        let formattedResults = results.map((result) => {
            ...
            if (!meta)
                return null;
            const wsName = m.workspaceName || meta.workspaceName || (meta.workspacePath && meta.workspacePath.split('/').pop()) || meta.workspacePath;
            if (workspaceFilter && workspaceFilter !== 'all' && wsName !== workspaceFilter) {
                return null;
            }
            const textForSnippet = m.text || '';
            ...
            return {
                ...
                messageCount: m.messageCount ?? meta.title.length,
    ...
    async handleGetConversation(id) {
        ...
        const messages = await (0, chatExtractor_1.getChatContentFromWorkspaceStorage)(this.extensionPath, meta.workspaceHash, meta.tabIndex, meta.composerId);
        this.panel.webview.postMessage({
            type: 'conversation',
            id,
            messages,
    ...
    async handleCopyToChat(id) {
        ...
        if (workspaceFolder) {
            const result = await (0, workspaceChatInjector_1.injectChatIntoCurrentWorkspace)(this.extensionPath, workspaceFolder, messages, meta.title);
            if (result.injected) {
                this.panel.webview.postMessage({
                    type: 'toast',
        ...
        const text = (0, workspaceChatInjector_1.formatConversationAsMarkdown)(messages, meta.title);
...
exports.ChatSearchPanel = ChatSearchPanel;
