...
function getBubblesForComposer(globalDb: Database, composerId: string): ChatMessage[] {
  ...
  try {
    ...
    const rawBubbles: Array<{type: number; text: string; createdAt?: number; order: number}> = [];
...
function getAllBubblesForWorkspace(globalDb: Database, composerIds: string[]): Array<{composerIndex: number; messages: ChatMessage[]; name: string}> {
  const results: Array<{composerIndex: number; messages: ChatMessage[]; name: string}> = [];
...
export async function extractDocumentsForSearch(
  ...
  for (let i = 0; i < dbDirs.length; i++) {
    ...
    if (i % 20 === 0) report(`Scanning ${i + 1}/${dbDirs.length}...`);
    try {
